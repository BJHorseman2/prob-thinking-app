...
import ChallengePathNavigator from '@/components/ChallengePathNavigator'
...
type BadgeId = 'first-challenge' | 'system-1-survivor' | 'crowd-beater' | 'bias-slayer' | 
  ...
  'nobel-laureate' | 'grand-master' | 'disciplined-trader' | 'market-historian' | 'sharp-bettor' |
  'diamond-hands' | 'crisis-alpha' | 'volatility-hunter'
...
export default function Probabl() {
  ...
  }, [userScore, streak, crowdBeats, badges, completedChallenges, completedMarkets, userRank])

  // Initialize user profile on first load if authenticated
  useEffect(() => {
    if (user?.id && user?.email && userScore === 0) {
      DataService.getUserProfile(user.id).then(profile => {
        if (!profile) {
          // Create new user profile
          DataService.createUserProfile(user.id, user.email)
        } else {
          // Load existing profile data
          setUserScore(profile.score)
          setStreak(profile.streak)
          setCrowdBeats(profile.crowd_beats)
        }
      })
    }
  }, [user, userScore])
  ...
  const badgeSystem: Record<BadgeId, Badge> = {
    ...
    // Behavioral Finance Challenge Path Badges
    'disciplined-trader': {
      name: 'Disciplined Trader',
      emoji: '🎯',
      description: 'Completed Rookie Trader Academy - survived first trades without blowing up',
      rarity: 'Rare',
      points: 500,
      nftMetadata: {
        background: 'gradient-gold',
    ...
    'market-historian': {
      name: 'Market Historian',
      emoji: '🏛️',
      description: 'Completed Wall Street Legends - learned from the greatest (and their mistakes)',
      rarity: 'Rare',
      points: 500,
      nftMetadata: {
        background: 'gradient-gold',
    ...
    'sharp-bettor': {
      name: 'Sharp Bettor',
      emoji: '🏈',
      description: 'Completed Sports Betting Psychology - mastered the mental game',
      rarity: 'Rare',
      points: 500,
      nftMetadata: {
        background: 'gradient-gold',
    ...
    'diamond-hands': {
      name: 'Diamond Hands',
      emoji: '💎',
      description: 'Completed Crypto Casino - survived the wild west of digital assets',
      rarity: 'Rare',
      points: 500,
      nftMetadata: {
        background: 'gradient-gold',
    ...
    'crisis-alpha': {
      name: 'Crisis Alpha',
      emoji: '📉',
      description: 'Completed Market Crash Survival - thrives when others panic',
      rarity: 'Rare',
      points: 500,
      nftMetadata: {
        background: 'gradient-gold',
    ...
    'volatility-hunter': {
      ...
      emoji: '🎲',
      description: 'Completed Options Assassin - masters the most dangerous weapons in finance',
      rarity: 'Rare',
      points: 500,
      nftMetadata: {
        background: 'gradient-gold',
        trait: 'Options Warrior',
        power: 12
      }
    }
  }
  ...
  const gameRules = {
    ...
    badges: {
      ...
      // Behavioral Finance Challenge Path Completion Badges
      'disciplined-trader': () => {
        const rookieIds = ['rookie-1', 'rookie-2', 'rookie-3', 'rookie-4', 'rookie-5']
        return rookieIds.every(id => completedChallenges.includes(id))
      },
      'market-historian': () => {
        const legendIds = ['legend-1', 'legend-2', 'legend-3', 'legend-4', 'legend-5']
        return legendIds.every(id => completedChallenges.includes(id))
      },
      'sharp-bettor': () => {
        const sportsIds = ['sports-1', 'sports-2', 'sports-3', 'sports-4', 'sports-5']
        return sportsIds.every(id => completedChallenges.includes(id))
      },
      'diamond-hands': () => {
        const cryptoIds = ['crypto-1', 'crypto-2', 'crypto-3', 'crypto-4', 'crypto-5']
        return cryptoIds.every(id => completedChallenges.includes(id))
      },
      'crisis-alpha': () => {
        const crashIds = ['crash-1', 'crash-2', 'crash-3', 'crash-4', 'crash-5']
        return crashIds.every(id => completedChallenges.includes(id))
      },
      'volatility-hunter': () => {
        const optionsIds = ['options-1', 'options-2', 'options-3', 'options-4', 'options-5']
        return optionsIds.every(id => completedChallenges.includes(id))
      }
  ...
  const checkForNewBadges = () => {
    (Object.keys(badgeSystem) as BadgeId[]).forEach(badgeId => {
      if (!badges.includes(badgeId) && gameRules.badges[badgeId]?.()) {
        ...
        const badgePoints = badgeSystem[badgeId].points
        setUserScore(prev => prev + badgePoints)
        
        // Update leaderboard if user is authenticated
        if (user?.id) {
          LeaderboardService.updateUserScore(user.id, badgePoints)
        }
  ...
  const liveMarkets = [
    {
      id: 1,
      question: "Will Bitcoin reach $150,000 before January 2026?",
      timeLeft: "147d 8h",
      participants: 234,
      averageGuess: 58,
      prize: "🏅 Prophet Badge",
  ...
  const handleChallengeAnswer = (challenge: any, answer: any) => {
    ...
    if (!isNaN(points) && points > 0) {
      ...
      // Update streak
      const newStreak = streak + (points > challenge.points * 0.7 ? 1 : 0)
      setStreak(newStreak)
      
      // Update crowd beats
      const newCrowdBeats = beatCrowd ? crowdBeats + 1 : crowdBeats
      if (beatCrowd) {
        setCrowdBeats(newCrowdBeats)
      ...
      if (user?.id) {
        ...
        // Assign initial rank when user gets their first points (fallback for non-authenticated)
  ...
  const handleMarketPrediction = () => {
    ...
    // Update leaderboard if user is authenticated
    if (user?.id) {
      LeaderboardService.updateUserScore(user.id, points)
    }
  ...
  return (
    <MobileLayout
      onSwipeUp={() => {
        ...
        const tabs = ['home', 'challenges', 'paths', 'markets', 'progress', 'badges', 'leaderboard']
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-indigo-900 to-black text-white">
      ...
      {activeTab === 'paths' && <ChallengePathNavigator />}
